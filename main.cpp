...
void startup(){
    uartUsb.write("\r\nEnter 0 on keypad to activate system:\r\n", 41);

    while (true){
        char key =  matrixKeypadScan();
        if (key == '0'){
            uartUsb.write("System activated\r\n", 18);
            break;
        }
        else if(key != '\0'){
            uartUsb.write("Invalid startup, Press 0\r\n",26);
        }
        delay(200);
    }
...
void dateTime(){
    if(!messagestatus){
        char warning[50];
        time_t event = time(NULL);
        sprintf(warning,"Alarm Triggered at: %s \r\n", ctime(&event));
        uartUsb.write(warning, 50);
        messagestatus = true;
    }
}
